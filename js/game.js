...
function init() {
    ...
    canvas.addEventListener("click", () => {
        if (musicOn && !musicInstance) attemptStartMusic();
    }, { once: true });

    document.addEventListener("keydown", (e) => {

        if (musicOn && !musicInstance) {
            attemptStartMusic();
        }
...
function createMusicButton() {
    ...
    const label = new createjs.Text(musicOn ? "Música: ON" : "Música: OFF", "20px Arial", "#fff");
    label.name = "label";
    label.x = 15;
    label.y = 8;
    ...
    if (player && !player.isDead) {
        for (let i = 0; i < obstacleManager.obstacles.length; i++) {
            ...
            if (Collision.checkRect(player, o)) {
                ...
                const deathDuration = (player.deadFrames && player.framesPerImage) ? player.deadFrames.length * player.framesPerImage * (1000 / 60) : 1000;
                player.isDead = true;
